    ...
    <script>
        $('#payment-form').on('submit', function(e) {
            ...
            $.post('gerar_qr.php', $(this).serialize(), function(response) {
                if (response.qr_code) {
                    $('#qr-code').html('<img src="data:image/png;base64,' + response.qr_code + '" alt="QR Code">');
                    const paymentId = response.payment_id;
                    $.get('verificar_pagamento.php', { payment_id: paymentId }, function(statusResponse) {
                        $('#payment-status').html('<p>' + statusResponse.status + '</p>');
                    }, 'json');
                } else {
